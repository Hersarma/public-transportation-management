<div class="relative flex flex-col mt-16">
 <div class="fixed top-0 left-0 right-0 lg:ml-64 z-10 flex-shrink-0 flex h-16 bg-gray-700">
      <button @click="open =! open" type="button" class="px-4 border-r border-gray-200 text-gray-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500 lg:hidden">
      ...
      <div class="flex-1 px-2 lg:px-8 flex justify-between">
        <div class="flex-1 flex">
          <div class="w-full flex md:ml-0">
             <a href="{{ route('homeAdmin') }}" class="mr-2 text-white group flex items-center px-2 py-2 text-sm leading-6 font-medium rounded-md {{ Request::is('admin') ? 'underline underline-offset-8 decoration-yellow-400' : '' }}" aria-current="page">
          ...
          <div x-data="{ open: false }" class="ml-2 relative">
            <div>
              <button @click="open =! open" type="button" class="max-w-xs rounded-full flex items-center text-sm lg:p-2" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                <span class="ml-1 text-white text-sm font-medium"><span class="sr-only">Open user menu for </span>{{ auth()->user()->name }}</span>
                <!-- Heroicon name: solid/chevron-down -->
                <svg class="hidden flex-shrink-0 ml-1 h-5 w-5 text-yellow-400 lg:block" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
