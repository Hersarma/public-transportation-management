    ...
    <div class="fixed inset-0 flex z-40">
      ...
      <div @click.away="open = false" x-show="open" x-transition:enter="transition ease-in-out duration-300 transform" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in-out duration-300 transform" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full" class="relative flex-1 flex flex-col max-w-xs w-full pt-5 pb-4 bg-gradient-to-b from-gray-700 to-gray-600">
        ...
        <nav class="mt-5 flex-shrink-0 h-full divide-y divide-yellow-400 overflow-y-auto" aria-label="Sidebar">
          <div x-data="{ open: false }" class="px-2 space-y-1">
            <button @click="open =! open" type="button" class="{{ Request::is('vehicles','vignettes','vehicleRegistrations','vehicles/*', 'vignettes/*', 'vehicleRegistrations/*') ? 'bg-gray-800' : '' }} w-full text-white hover:text-white hover:bg-gray-800 group flex items-center px-2 py-2 text-left text-sm leading-6 font-medium rounded-md" aria-expanded="false">
          ...
          <span class="flex-1"> Vozni Park </span>
          <!-- Expanded: "text-gray-400 rotate-90", Collapsed: "text-gray-300" -->
          <svg class="text-yellow-400 ml-3 flex-shrink-0 h-5 w-5 transform group-hover:text-gray-400 transition-colors ease-in-out duration-150" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
        ...
        <div x-cloak x-show="open" class="space-y-1">
          <a href="{{ route('vehicles.index') }}" class="{{ Request::is('vehicles') ? 'bg-gray-800' : '' }} group w-full flex items-center pl-11 pr-2 py-2 text-sm font-medium rounded-md text-white hover:text-white hover:bg-gray-800">
           Vozila </a>
          <a href="{{ route('vehicleRegistrations.index') }}" class="{{ Request::is('vehicleRegistrations', 'vehicleRegistrations/*') ? 'bg-gray-800' : '' }} group w-full flex items-center pl-11 pr-2 py-2 text-sm font-medium rounded-md text-white hover:text-white hover:bg-gray-800">
           Registracije </a>
          <a href="{{ route('vignettes.index')}}" class="{{ Request::is('vignettes', 'vignettes/*') ? 'bg-gray-800' : '' }} group w-full flex items-center pl-11 pr-2 py-2 text-sm font-medium rounded-md text-white hover:text-white hover:bg-gray-800"> 
          Vinjete </a>
          ...
          Održavanje </a>
            ...
            <a href="{{ route('clients.index') }}" class="{{ Request::is('clients', 'clients/*') ? 'bg-gray-800' : '' }} text-white hover:text-white hover:bg-gray-800 group flex items-center px-2 py-2 text-base font-medium rounded-md">
            ...
            <a href="{{ route('gpsAdmin') }}" class="{{ Request::is('gps') ? 'bg-gray-800' : '' }} text-white hover:text-white hover:bg-gray-800 group flex items-center px-2 py-2 text-base font-medium rounded-md">
          ...
          <div class="mt-6 pt-6">
            <div class="px-2 space-y-1">
              ...
              <a href="{{ route('homeDrivers') }}" class="group flex items-center px-2 py-2 text-base font-medium rounded-md text-white hover:text-white hover:bg-gray-800">
    ...
    <div class="flex flex-col flex-grow bg-gradient-to-b from-gray-700 to-gray-600 pt-2 pb-2 overflow-y-auto">
      ...
      <nav class="mt-5 flex-1 flex flex-col divide-y divide-yellow-400 overflow-y-auto" aria-label="Sidebar">
        ...
        <div x-cloak x-show="open" class="space-y-1">
          ...
          <a href="{{ route('vehicles.index') }}" class="{{ Request::is('vehicles', 'vehicles/*') ? 'bg-gray-800' : '' }} group w-full flex items-center pl-11 pr-2 py-2 text-sm font-medium rounded-md text-white hover:text-white hover:bg-gray-800">
           Vozila </a>
          <a href="{{ route('vehicleRegistrations.index') }}" class="{{ Request::is('vehicleRegistrations', 'vehicleRegistrations/*') ? 'bg-gray-800' : '' }} group w-full flex items-center pl-11 pr-2 py-2 text-sm font-medium rounded-md text-white hover:text-white hover:bg-gray-800">
           Registracije </a>
          <a href="{{ route('vignettes.index')}}" class="{{ Request::is('vignettes', 'vignettes/*') ? 'bg-gray-800' : '' }} group w-full flex items-center pl-11 pr-2 py-2 text-sm font-medium rounded-md text-white hover:text-white hover:bg-gray-800"> 
          ...
          <a href="{{ route('drivers.index') }}" class="{{ Request::is('drivers', 'drivers/*') ? 'bg-gray-800' : '' }} text-white hover:text-white hover:bg-gray-800 group flex items-center px-2 py-2 text-sm leading-6 font-medium rounded-md">
          ...
          <a href="{{ route('clients.index') }}" class="{{ Request::is('clients', 'clients/*') ? 'bg-gray-800' : '' }} text-white hover:text-white hover:bg-gray-800 group flex items-center px-2 py-2 text-sm leading-6 font-medium rounded-md">
            <!-- Heroicon name: outline/credit-card -->
            <svg xmlns="http://www.w3.org/2000/svg" class="mr-4 flex-shrink-0 h-6 w-6 text-yellow-400" viewBox="0 0 20 20" fill="currentColor">
          ...
          <a href="{{ route('gpsAdmin') }}" class="{{ Request::is('gpsAdmin') ? 'bg-gray-800' : '' }} text-white hover:text-white hover:bg-gray-800 group flex items-center px-2 py-2 text-sm leading-6 font-medium rounded-md">
            <svg xmlns="http://www.w3.org/2000/svg" class="mr-4 flex-shrink-0 h-6 w-6 text-yellow-400" viewBox="0 0 20 20" fill="currentColor">
              <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
            </svg>
            GPS
        ...
        <div class="mt-6 pt-6">
          <div class="px-2 space-y-1">
            ...
            <a href="{{ route('homeDrivers') }}" class="group flex items-center px-2 py-2 text-sm leading-6 font-medium rounded-md text-white hover:text-white hover:bg-gray-800">
              <svg class="mr-4 h-6 w-6 text-yellow-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
              </svg>
              Panel za vozače
