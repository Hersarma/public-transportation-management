...
<div class="bg-white shadow overflow-hidden sm:rounded-lg lg:mx-8 lg:my-8">
  <div class="px-4 py-6 sm:px-6 flex flex justify-between items-center">
    <h3 class="text-lg leading-6 font-medium text-gray-900">Informacije o vinjeti</h3>
    <div
      @if(count($errors->edit_vignette) > 0)
      x-data="{open: true}"
      @endif
      x-data="{open: false}" class="">
      <button @click="open = true" type="button" class="inline-flex items-center justify-center rounded-md border border-transparent bg-gray-700 px-4 py-2 text-sm font-medium text-yellow-400 shadow-sm hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2 sm:w-auto">Izmeni vinjetu<span class="ml-2">
  ...
  <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
    ...
    <dl class="sm:divide-y sm:divide-gray-200">
      ...
      <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
        <dt class="text-sm font-medium text-gray-500">Datum kupovine</dt>
        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{$vignette->purchase_date->format('d-m-Y') }}</dd>
      </div>
      <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
        <dt class="text-sm font-medium text-gray-500">Važi do</dt>
        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{$vignette->expiration_date->format('d-m-Y')}}</dd>
      </div>
      
      
      <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
        <dt class="text-sm font-medium text-gray-500">Država</dt>
        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $vignette->country }}</dd>
      </div>
      <div x-data="{open: false}">
        <div class="hidden sm:block py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
          ...
          <dl>
            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $vignette->price }} din.</dd>
            @if(!empty($vignette->receipt))
        ...
        <div class="sm:hidden flex items-center justify-between py-4">
          <div>
            ...
            <p class="mt-1 text-sm text-gray-900">{{ $vignette->price }} din.</p>
          </div>
        ...
        @include('admin.vignettes.modals.receipt')
      </div>
